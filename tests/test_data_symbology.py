...
class TestSymbologyService(unittest.TestCase):
    ...
    def test_multi_id_mapping(self):
        ...
        self.ism.add_external_mapping(internal_id, "FIGI", "BBG000B9XRY4", start_date)
        self.ism.add_external_mapping(internal_id, "ISIN", "US0378331005", start_date)

        # Test mapping from FIGI
        id_figi = self.service.map_to_internal_id("FIGI", "BBG000B9XRY4", datetime(2020, 6, 1))
        self.assertEqual(id_figi, internal_id)

        # Test mapping from ISIN
        id_isin = self.service.map_to_internal_id("ISIN", "US0378331005", datetime(2020, 6, 1))
        self.assertEqual(id_isin, internal_id)

        # Test get all identifiers
        all_ids = self.service.get_all_identifiers(internal_id, datetime(2020, 6, 1))
    ...
    def test_ticker_lookup(self):
        ...
        id_ticker = self.service.map_to_internal_id("TICKER", "TSLA", datetime(2020, 6, 1), exchange="NASDAQ")
        self.assertEqual(id_ticker, internal_id)

        with self.assertRaises(ValueError):
            self.service.map_to_internal_id("TICKER", "TSLA", datetime(2020, 6, 1))
