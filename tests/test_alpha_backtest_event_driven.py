...
class TestEventDrivenBacktester(unittest.TestCase):
    def setUp(self):
        ...
        self.market_data = MarketDataEngine(os.path.join(self.test_dir, "market"))
    ...
    def test_backtest_run(self):
        ...
        for i in range(10):
            ...
            bars.append(Bar(internal_id=1, timestamp=ts, timestamp_knowledge=ts, 
                            open=100.0+i, high=101.0+i, low=99.0+i, close=100.5+i, volume=1000.0))
            # ID 2 goes down: 100, 99, 98...
            bars.append(Bar(internal_id=2, timestamp=ts, timestamp_knowledge=ts, 
                            open=100.0-i, high=101.0-i, low=99.0-i, close=99.5-i, volume=1000.0))
