...
class TestInternalSecurityMaster(unittest.TestCase):
    ...
    def test_registration_and_lookup(self):
        ...
        # Lookup by ticker
        found_id = self.ism.get_internal_id("AAPL", "NASDAQ", datetime(2020, 6, 1))
    ...
    def test_ticker_change(self):
        ...
        internal_id = self.ism.register_security("FB", "NASDAQ", datetime(2012, 5, 18))
        
        # Ticker change
        self.ism.register_security("META", "NASDAQ", datetime(2022, 6, 9), internal_id=internal_id)
        ...
        id_new = self.ism.get_internal_id("META", "NASDAQ", datetime(2023, 1, 1))
        self.assertEqual(id_new, internal_id)
        
        # Test old ticker after change (should be None or handled)
        id_old_after = self.ism.get_internal_id("FB", "NASDAQ", datetime(2023, 1, 1))
        self.assertIsNone(id_old_after)

    def test_invalid_lookup(self):
        found_id = self.ism.get_internal_id("INVALID", "NONE", datetime(2020, 1, 1))
        self.assertIsNone(found_id)

    def test_get_universe(self):
        id1 = self.ism.register_security("AAPL", "NASDAQ", datetime(2020, 1, 1))
        id2 = self.ism.register_security("TSLA", "NASDAQ", datetime(2020, 1, 1))
