    ...
    def test_full_backtest_run(self):
        ...
        for iid, base_price in [(1, 100.0), (2, 200.0)]:
            ...
            for i in range(num_days):
                ...
                price = base_price + np.sin(i * 0.5) * 5 + np.random.normal(0, 1)
                bars.append({
                    "internal_id": iid,
                    "timestamp": ts,
                    "open": price,
        ...
        # 2. Setup components
        alpha_model = MeanReversionModel(self.feature_store, internal_ids=[1, 2])
        risk_model = RollingWindowRiskModel(market_data_engine=self.mde, window_days=10)
        ...
        backtester = EventDrivenBacktester(
            ...
            optimizer=optimizer,
            start_date=start_date + timedelta(days=10), # Start after we have some data for models
