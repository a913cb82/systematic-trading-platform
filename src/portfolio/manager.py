...
class PortfolioManager:
    def __init__(self, forecast_publisher: ForecastPublisher, optimizer: PortfolioOptimizer, weight_publisher: TargetWeightPublisher):
    ...
    def on_forecast(self, timestamp: datetime, forecasts: Dict[int, float]) -> None:
        """
        Callback triggered when new forecasts are available.
        Runs optimization and publishes target weights.
        """
        target_weights = self.optimizer.optimize(timestamp, forecasts, self.current_weights)
