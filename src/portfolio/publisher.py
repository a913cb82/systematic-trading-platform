...
class TargetWeightPublisher:
    ...
    def submit_target_weights(self, timestamp: datetime, weights: Dict[int, float]) -> None:
    ...
    def get_target_weights(self, timestamp: datetime) -> Dict[int, float]:
        ...
        weights = {}
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.execute("SELECT internal_id, weight FROM target_weights WHERE timestamp = ?", (ts_str,))
            for row in cursor.fetchall():
                weights[row[0]] = row[1]
        return weights

    def subscribe_target_weights(self, on_weights: Callable[[datetime, Dict[int, float]], None]) -> None:
