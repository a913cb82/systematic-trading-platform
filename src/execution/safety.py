...
class SafetyLayer:
    ...
    def check_rate_limit(self) -> bool:
        ...
        self.message_timestamps = [t for t in self.message_timestamps if now - t < timedelta(seconds=1)]
    ...
    def update_pnl(self, current_equity: float):
        ...
        if pnl_pct < self.max_drawdown_limit:
            self.is_killed = True
            print(f"CRITICAL: Kill-switch triggered. Drawdown {pnl_pct:.2%} exceeds limit {self.max_drawdown_limit:.2%}")
    ...
    def validate_weights(self, weights: Dict[int, float]) -> Tuple[bool, str]:
        ...
        if not self.check_rate_limit():
            return False, f"Message rate limit of {self.max_messages_per_second}/s exceeded."
