...
class ExecutionAlgorithm:
    ...
    def simulate_fills(self, timestamp: datetime, target_weights: Dict[int, float], 
                       current_portfolio_value: float = 1_000_000.0) -> List[Trade]:
        """
        ...
        bars = self.market_data_engine.get_bars(internal_ids, timestamp, timestamp)
        ...
        for iid, weight in target_weights.items():
            if iid in bar_map:
                ...
                fill = Trade(
                    ...
                    quantity=abs(weight * current_portfolio_value / bar['close']),
                    price=bar['close'],
                    fees=0.0,
                    venue='SIM',
                    timestamp=timestamp
