...
class SignalCombiner:
    @staticmethod
    def equal_weight(forecast_list: List[Dict[int, float]]) -> Dict[int, float]:
        ...
        return {iid: combined[iid] / len(forecast_list) for iid in combined.keys()}

    @staticmethod
    def weighted_average(forecast_list: List[Dict[int, float]], weights: List[float]) -> Dict[int, float]:
