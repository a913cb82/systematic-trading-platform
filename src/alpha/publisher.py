...
class ForecastPublisher:
    ...
    def submit_forecasts(self, timestamp: datetime, forecasts: Dict[int, float]) -> None:
    ...
    def get_forecasts(self, timestamp: datetime) -> Dict[int, float]:
        ...
        forecasts = {}
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.execute("SELECT internal_id, signal FROM forecasts WHERE timestamp = ?", (ts_str,))
            for row in cursor.fetchall():
                forecasts[row[0]] = row[1]
        return forecasts

    def subscribe_forecasts(self, on_forecast: Callable[[datetime, Dict[int, float]], None]) -> None:
