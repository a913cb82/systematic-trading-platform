...
class FeatureStore:
    ...
    def calculate_cycle_feature(self, internal_ids: List[int], start: datetime, end: datetime, feature_name: str) -> pd.DataFrame:
        """
    ...
    def calculate_event_feature(self, internal_ids: List[int], start: datetime, end: datetime, feature_name: str) -> pd.DataFrame:
        """
        Calculates a feature based on event data.
        Example: 'earnings_surprise', 'sentiment_score'
        """
        if feature_name == 'earnings_surprise':
            events = self.event_store.get_events(['EARNINGS'], internal_ids, start, end)
            if not events:
                return pd.DataFrame()
            df = pd.DataFrame(events)
            # Assume value is dict with 'surprise'
            df['feature'] = df['value'].apply(lambda x: x.get('surprise', 0) if isinstance(x, dict) else 0)
            return df[['internal_id', 'timestamp_event', 'feature']].rename(columns={'timestamp_event': 'timestamp'})
        else:
