...
class CorporateActionMaster:
    ...
    def _init_db(self):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            """)
            conn.execute("CREATE INDEX IF NOT EXISTS idx_ca_id ON corporate_actions (internal_id, ex_date)")
        ...
        with sqlite3.connect(self.db_path) as conn:
            for action in actions:
                ...
                """, (
                    ...
                    action['record_date'].strftime("%Y-%m-%d") if action['record_date'] else None,
                    action['pay_date'].strftime("%Y-%m-%d") if action['pay_date'] else None,
                    action['ratio'],
                    action['timestamp_knowledge'].strftime("%Y-%m-%d %H:%M:%S")
                ))

    def get_actions(self, internal_ids: List[int], start: datetime, end: datetime, as_of: Optional[datetime] = None) -> List[CorporateAction]:
        if as_of is None:
            as_of = datetime.now()
                ...
                for row in cursor.fetchall():
                    ...
                    if key not in seen:
                        ...
                        actions.append(CorporateAction(
                            ...
                            record_date=datetime.strptime(row[2], "%Y-%m-%d") if row[2] else None,
                            pay_date=datetime.strptime(row[3], "%Y-%m-%d") if row[3] else None,
                            ratio=row[4],
                            timestamp_knowledge=datetime.strptime(row[5], "%Y-%m-%d %H:%M:%S")
                        ))
        return actions
