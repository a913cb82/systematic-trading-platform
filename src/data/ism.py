...
class InternalSecurityMaster:
    ...
    def register_security(self, ticker: str, exchange: str, start_date: datetime, sector: Optional[str] = None, internal_id: Optional[int] = None) -> int:
        """
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            conn.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?, ?)
            """, (internal_id, ticker, exchange, sector, start_date_str, None, now_str))
    ...
    def add_external_mapping(self, internal_id: int, id_type: str, id_value: str, start_date: datetime):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            conn.execute("""
                ...
                VALUES (?, ?, ?, ?, ?, ?)
            """, (internal_id, id_type, id_value, start_date_str, None, now_str))

    def get_internal_id(self, ticker: str, exchange: str, date: datetime, as_of: Optional[datetime] = None) -> Optional[int]:
    ...
    def get_internal_id_by_external(self, id_type: str, id_value: str, date: datetime, as_of: Optional[datetime] = None) -> Optional[int]:
        date_str = date.strftime("%Y-%m-%d")
        as_of_str = (as_of or datetime.now()).strftime("%Y-%m-%d %H:%M:%S")
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.execute("""
    ...
    def get_symbol_info(self, internal_id: int, date: datetime, as_of: Optional[datetime] = None) -> Optional[Dict[str, Any]]:
        date_str = date.strftime("%Y-%m-%d")
        as_of_str = (as_of or datetime.now()).strftime("%Y-%m-%d %H:%M:%S")
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.execute("""
    ...
    def get_external_mappings(self, internal_id: int, date: datetime, as_of: Optional[datetime] = None) -> Dict[str, str]:
        date_str = date.strftime("%Y-%m-%d")
        as_of_str = (as_of or datetime.now()).strftime("%Y-%m-%d %H:%M:%S")
        mappings = {}
        with sqlite3.connect(self.db_path) as conn:
    ...
    def get_universe(self, date: datetime, as_of: Optional[datetime] = None) -> list[int]:
        date_str = date.strftime("%Y-%m-%d")
        as_of_str = (as_of or datetime.now()).strftime("%Y-%m-%d %H:%M:%S")
        with sqlite3.connect(self.db_path) as conn:
            cursor = conn.execute("""
