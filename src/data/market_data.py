...
class MarketDataEngine(MarketDataProvider):
    ...
    def write_bars(self, data: List[Bar]) -> None:
        ...
        for internal_id, group in df.groupby('internal_id'):
            id_path = os.path.join(self.bars_path, f"internal_id={internal_id}")
            ...
            if os.path.exists(file_path):
                ...
                combined_df = combined_df.sort_values(['timestamp', 'timestamp_knowledge'])
                combined_df.to_parquet(file_path, index=False)
            else:
                group.sort_values(['timestamp', 'timestamp_knowledge']).to_parquet(file_path, index=False)
    ...
    def get_bars(self, internal_ids: List[int], start: datetime, end: datetime, adjustment: str = "RAW", as_of: Optional[datetime] = None) -> List[Bar]:
        all_bars = []
        if as_of is None:
            as_of = datetime.now()
        ...
        for internal_id in internal_ids:
            ...
            if os.path.exists(file_path):
                ...
                df = df.sort_values('timestamp_knowledge').groupby('timestamp').last().reset_index()
    ...
    def subscribe_bars(self, internal_ids: List[int], on_bar: Callable[[Bar], None]) -> None:
