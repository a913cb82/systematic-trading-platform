...
class EventStore:
    ...
    def write_events(self, events: List[Event]) -> None:
        ...
        for (event_type, internal_id), group in df.groupby(['type', 'internal_id']):
            type_path = os.path.join(self.base_path, f"type={event_type}", f"internal_id={internal_id}")
            ...
            if os.path.exists(file_path):
                ...
                existing_df['timestamp_event'] = pd.to_datetime(existing_df['timestamp_event'])
                existing_df['timestamp_knowledge'] = pd.to_datetime(existing_df['timestamp_knowledge'])
                combined_df = pd.concat([existing_df, group])
                combined_df = combined_df.sort_values(['timestamp_event', 'timestamp_knowledge'])
                combined_df.to_parquet(file_path, index=False)
            else:
                group.sort_values(['timestamp_event', 'timestamp_knowledge']).to_parquet(file_path, index=False)
    ...
    def get_events(self, event_types: List[str], internal_ids: List[int], start: datetime, end: datetime, as_of: Optional[datetime] = None) -> List[Event]:
        all_events = []
        if as_of is None:
            as_of = datetime.now()
        ...
        for event_type in event_types:
            for internal_id in internal_ids:
                ...
                if os.path.exists(file_path):
                    ...
                    df = df.sort_values('timestamp_knowledge').groupby('timestamp_event').last().reset_index()
                    
                    mask = (df['timestamp_event'] >= start) & (df['timestamp_event'] <= end)
    ...
    def subscribe_events(self, event_types: List[str], on_event: Callable[[Event], None]) -> None:
